...
class DQN(torch.nn.Module):
    ...
    def __init__(self, output_size, in_channels):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=in_channels,
                                     out_channels=2,
                                     kernel_size=3)
        self.conv2 = torch.nn.Conv2d(in_channels=2,
                                     out_channels=1,
                                     kernel_size=3)
...
class BattleSnakeEnv(gym.Env):
    ...
    def _get_info(self):
        return {
            "len_snake": len(self.game_state["you"]["body"]),
            "food_number": len(self.game_state["board"]["food"])
        }
    ...
    def take_action(self, action, snake_idx):
        ...
        if next_pos_head["y"] == 11 or next_pos_head[
                "y"] == -1 or next_pos_head["x"] == -1 or next_pos_head[
                    "x"] == 11:
            ...
            for index, food in enumerate(foods):
                if next_pos_head["x"] == food["x"] and next_pos_head[
                        "y"] == food["y"]:
                    test = True
                    food_index = index
                    break
            ...
            if (test1):
                ...
                if (test):
                    ...
                    tail_pos = self.game_state["board"]["snakes"][snake_idx][
                        "body"][-1]
                    if tail_pos == self.game_state["board"]["snakes"][
                            snake_idx]["body"][1]:
                        self.game_state["board"]["snakes"][snake_idx][
                            "body"] = [{
                                "x": next_pos_head["x"],
                                "y": next_pos_head["y"]
                            }, tail_pos, tail_pos]
                    elif len(self.game_state["board"]["snakes"][snake_idx]
                             ["body"]) >= 4:
                        if self.game_state["board"]["snakes"][snake_idx][
                                "body"][-1] == self.game_state["board"][
                                    "snakes"][snake_idx]["body"][-2]:
                            self.game_state["board"]["snakes"][snake_idx][
                                "body"].pop(-1)
                    self.game_state["board"]["snakes"][snake_idx][
                        "length"] += 1
                    ...
                    tail_pos = self.game_state["board"]["snakes"][snake_idx][
                        "body"][-1]
                    index_tail = len(self.game_state["board"]["snakes"]
                                     [snake_idx]["body"]) - 1
                    for index_seg, segment in enumerate(
                            self.game_state["board"]["snakes"][snake_idx]
                        ["body"]):
                        if segment == tail_pos:
                            ...
                            break
                    self.game_state["board"]["snakes"][snake_idx][
                        "body"] = self.game_state["board"]["snakes"][
                            snake_idx]["body"][:index_tail]
                    while (len(self.game_state["board"]["snakes"][snake_idx]
                               ["body"]) < 3):
                        self.game_state["board"]["snakes"][snake_idx][
                            "body"].append(self.game_state["board"]["snakes"]
                                           [snake_idx]["body"][-1])
                    self.game_state["board"]["snakes"][snake_idx][
                        "health"] -= 1

    def check_hit_snake_head_to_head(self, snake_id, other_snake_id):
        if self.previous_pos_heads[snake_id] == self.game_state["board"][
                "snakes"][other_snake_id]["body"][
                    0] and self.previous_pos_heads[
                        other_snake_id] == self.game_state["board"]["snakes"][
                            snake_id]["body"][0]:
            return True
        return self.game_state["board"]["snakes"][snake_id]["body"][
            0] == self.game_state["board"]["snakes"][other_snake_id]["body"][0]

    def check_hit_snake_body(self, snake_id, other_snake_id):
        head_pos = self.game_state["board"]["snakes"][snake_id]["body"][0]
        for _, segment in self.game_state["board"]["snakes"][other_snake_id][
                "body"]:
            if segment == head_pos:
                return True
        return False
    ...
    def snake_survives(self, snake_id):
        if len(
                self.game_state["board"]["snakes"][snake_id]["body"]
        ) == 0 or self.game_state["board"]["snakes"][snake_id]["health"] == 0:
            return False
        for other_snake_id in range(self.number_snakes):
            if len(self.game_state["board"]["snakes"][other_snake_id]["body"]
                   ) == 0 or other_snake_id == snake_id or self.game_state[
                       "board"]["snakes"][other_snake_id]["health"] == 0:
                continue
            test_hit_other_snake_head_to_head = self.check_hit_snake_head_to_head(
                snake_id, other_snake_id)
            #if (test_hit_other_snake_head_to_head):
            #print(f"snake {snake_id} : hit another snake head to head")
            length_snake = self.game_state["board"]["snakes"][snake_id][
                "length"]
            length_other_snake = self.game_state["board"]["snakes"][
                other_snake_id]["length"]
            if test_hit_other_snake_head_to_head:
                if length_snake >= length_other_snake:
                    self.game_state["board"]["snakes"][other_snake_id][
                        "body"] = []
                    self.game_state["board"]["snakes"][other_snake_id][
                        "length"] = 0
                    self.game_state["board"]["snakes"][other_snake_id][
                ...
                test_hit_other_snake_body = self.check_hit_snake_body(
                    snake_id, other_snake_id)
    ...
    def step(self, action):
        ...
        if not main_snake_survives:
            ...
            reward = -1
            info = self._get_info()
            return self.game_state_to_env_state(
            ...
            self.previous_pos_heads[0] = self.game_state["board"]["snakes"][0][
                "body"][0]
            if (snake1_survives):
                self.previous_pos_heads[1] = self.game_state["board"][
                    "snakes"][1]["body"][0]
            if (snake2_survives):
                self.previous_pos_heads[2] = self.game_state["board"][
                    "snakes"][2]["body"][0]
            if (snake3_survives):
                self.previous_pos_heads[3] = self.game_state["board"][
                    "snakes"][3]["body"][0]
    ...
    def game_state_to_env_state(self):
        ...
        for food in self.game_state['board']['food']:
            state[(height - 1 - food['y']), food['x'],
                  0] = 1  # Mark food with 1
...
gym.register(id="gymnasium_env/BattleSnakeEnv", entry_point=BattleSnakeEnv)
"""pygame.init()
...
env = gym.make("gymnasium_env/BattleSnakeEnv",
               game_state=game_state,
               size=11,
               render_mode="rgb_array")  # replace with your environment
...
input_size = env.observation_space.shape[
    0]  # 4 inputs: position, velocity, angle, angular velocity
...
def test_dqn_agent(episodes=1):
    for episode in range(episodes):
        ...
        while not done:
            ...
            plt.clf()
            action = np.argmax(
                policy_net(torch.FloatTensor(np.transpose(
                    state, (2, 0, 1)))).detach().numpy())
...
for episode in range(episodes):
    ...
    while not done:
        ...
        if random.uniform(0, 1) < epsilon:
            ...
            action = np.argmax(
                policy_net(torch.FloatTensor(np.transpose(
                    state, (2, 0, 1)))).detach().numpy())  # Exploit
        ...
        if not done:
            ...
            if len(replay_buffer) > batch_size:
                ...
                # Calculate Q-values
                current_q_values = policy_net(states.permute(
                    (0, 3, 1, 2))).gather(1, actions.unsqueeze(1)).squeeze(1)
                next_q_values = target_net(next_states.permute(
                    (0, 3, 1, 2))).max(1)[0]
